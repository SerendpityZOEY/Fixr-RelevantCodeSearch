...
class Cards extends React.Component{
    ...
    render(){
        ...
        if(this.props.newCode!=''){
            ...
            //remove ();
            for(var i=0;i<queries.length;i++) queries[i] = queries[i].substr(0,queries[i].indexOf('('));
        ...
        CommitObjs = this.props.commit.map(function(s,i){
            ...
            return <Card style={style.cards} key={i}>
                ...
                <CardActions>
                    <FlatButton label="Detail" labelStyle={{fontSize:12}} onClick={() => this.handleChange(s)}/>
                    <FlatButton label="Fix" labelStyle={{fontSize:12}} onClick={() => this.handleClick(this.props.newCode, queries, s)}/>
                </CardActions>
            </Card>
        }, this);
