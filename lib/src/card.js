...
const add_1 = <Card style={style.cards}>
    <CardHeader
        title={'Cluster'}
        style={{
            height:50,
...
const trend = <Card style={style.cards}>
    <CardHeader
        title={'Trending'}
        style={{
            height:50,
    ...
    <CardText style={{
        ...
        overflowX:'hidden',
        padding:0
    }}><TinyLineChart/></CardText>
    <CardActions>
        <FlatButton label="Detail" style={{minWidth:8}} labelStyle={{fontSize:12}} onClick={() => this.handleChange(data, 'trend')}/>
        <FlatButton label="Fix" style={{minWidth:8}} labelStyle={{fontSize:12}}/>
    </CardActions>
</Card>;
...
class Cards extends React.Component{
    ...
    render(){
        ...
        CommitObjs = this.props.commit.map(function(s,i){
            ...
            if(i%3==0){
                return <span>{add_1}<Card style={style.cards} key={i}>
                    <CardHeader
                title={repo}
                subtitle={user}
            ...
            <CardText style={style.cardtext}>{msg}</CardText>
                ...
                return <span>{trend}<Card style={style.cards} key={i}>
                    <CardHeader
                        title={repo}
                        subtitle={user}
                        avatar={<TinyBarChart data={[{name:'Page A', add: add, remove: remove},]}/>}
        ...
        return <div style={style.cards}>{CommitObjs}{add_2}</div>;
    }
}
