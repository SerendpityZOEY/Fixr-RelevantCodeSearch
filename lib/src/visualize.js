...
class Visualize extends React.Component{
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            lastAction:'' //lines or fix
        };
        this.onAddBtnClick = this.onAddBtnClick.bind(this);
    }

    onAddBtnClick(val, multiSelection, lastAction, lineNum) {
        console.log('val from visualize', val);
        const inputList = this.state.inputList;
        if(multiSelection=='on' || lineNum==this.state.lineNum){
            inputList.pop();
        }
        const tmp = <NewRound key={this.state.index} initialCode={val} lineNum={this.state.lineNum+1}/>; //This is not an ideal solution
        this.setState({
    ...
    render(){
        ...
        if(this.props.solrConnector.response!=null){
            ...
            if(numFound<10) numFound+=numFound%3+2;
        ...
        if(this.state.hiddenCard) var commitDetail = null;
                                          ...
                                          lastAction={this.state.lastAction} lineNum={this.props.lineNum}
        />;
        ...
        <Editor doSearch={this.props.doSearch.bind(this)} initialCode={this.state.initialCode}
                getPrevEditorCode={this.getPrevEditorCode.bind(this)} lineNum={this.props.lineNum}
        />
        </Col>
        <Col xs={12} md={8} lg={8}>
            <Subheader style={{height:'3vw', marginTop:'-0.8vw'}}>{numFound} results fetched.</Subheader>
