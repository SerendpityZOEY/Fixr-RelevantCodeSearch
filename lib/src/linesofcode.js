...
class Lines extends React.Component{
    ...
    handleClick(val) {
        ...
        }else if(this.props.newCode.length>0 && this.state.multiSelection==false && this.props.lastAction==''){
            this.props.onAddBtnClick(input + '\n' + '//' + val.substr(1).trim(), 'off', 'lines', this.props.lineNum);
            this.setState({
                multiSelection: true,
            })
        }else{
            this.props.onAddBtnClick(this.props.newCode + '\n' + '//' + val.substr(1).trim(), 'on', 'lines', this.props.lineNum);
        }
    }
